...
def process_raw_data(raw_data_path, rules):
    ...
    for file_path in data_files:
        try:
            ...
            file_products = len([d for d in all_data[initial_data_count:] if d['Competitor'] == competitor])
            print(f"  -> Processed {idx + 1} rows, found {file_products} valid products for {competitor}")
